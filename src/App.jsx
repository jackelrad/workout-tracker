...
const gW = (w1, t) => [...PROG[t].map(d => r25(w1 + d)), r25(w1 * 0.85)];
const gPU = maxR => { const a = maxR >= 12 ? 15 : maxR >= 8 ? 10 : maxR >= 5 ? 5 : 0; return [0,0,0,0,a,a+5,a+10,a+10,a+15,a+20,a+25,0]; };

function buildWeights(b, eq) {
  if (!b || !Object.keys(b).length) return null;
  const hasPullup = eq?.pullup_bar !== false;
  return {
    bench:        gW(b.bench        || 135,  'compound_major'),
    pushdown:     gW(b.pushdown     || 32.5, 'isolation'),
    incline:      gW(b.incline      || 45,   'compound_minor'),
    skull:        gW(b.skull        || 50,   'isolation'),
    fly:          gW(b.fly          || 17.5, 'cable_light'),
    kickback:     gW(b.kickback     || 15,   'cable_light'),
    pullup:       hasPullup ? gPU(b.pullup_reps || 8) : Array(12).fill(0),
    row:          gW(b.row          || 65,   'compound_major'),
    facepull:     gW(b.facepull     || 40,   'isolation'),
    ...
    rdl:          gW(b.rdl          || 110,  'compound_minor'),
    calf:         gW(b.calf         || 90,   'isolation'),
    leg_ext:      gW(b.leg_ext      || 110,  'isolation'),
    leg_curl:     gW(b.leg_curl     || 70,   'isolation'),
  };
}
...
async function saveGeneratedWeights(userId, weights) {
  ...
  for (let i = 0; i < rows.length; i += 50) {
    await supabase.from("weight_adjustments").upsert(rows.slice(i, i+50), { onConflict:"user_id,day,week,exercise_id" });
  }
}
...
const FORM_CUES = {
  ...
  skull:        { setup:"Lie flat, EZ bar over forehead. Elbows shoulder-width apart, pointed at the ceiling.", cues:"Lower bar just behind your forehead. Elbows stay pointing up throughout — flaring is the key failure point. Controlled down, press up. 10lb increments only." },
  ...
  calf:         { setup:"Stand on a plate or step with heels hanging off the edge. Hold DBs or plates at your sides. Knee stays straight throughout.", cues:"Full stretch at the bottom — heel fully below the step, hold 1s. Drive all the way to the top — hold 1s. Full ROM is more important than load." },
  leg_ext:      { setup:"Seat adjusted so knee joint aligns with machine pivot. Pad on lower shin. Both legs simultaneously.", cues:"Extend fully, squeeze at top for 1 count. Slow eccentric — 2-3s down. Avoid swinging or momentum. Keep back against the pad." },
...
const MOBILITY = {
  chest_tri: [
    { name:"Thoracic Rotation", timing:"5 reps/side · ~45s", instructions:"Sit or kneel with hands behind your head. Slowly rotate your upper back as far as you can each direction. Feel your mid-back — not your lower back — doing the work." },
  ...
  legs: [
    ...
    { name:"Deep Squat Hold", timing:"2 × 20s · ~60s", instructions:"Hold a rack and lower into the deepest squat you can maintain with a flat back. Let hips sink, knees track over toes. Two holds, no bouncing." }
  ]
};

// ── PLAN DATA (default / Jack-calibrated) ─────────────
const PLAN = {
  chest_tri: {
  ...
  legs: {
    ...
    groups:[
      { label:"Straight Sets", rest:"Rest 2 min", supersetted:false, exercises:[
        ...
        { id:"leg_press", name:"Leg Press (total load)",       weights:[300,340,370,360,410,440,470,500,530,560,590,300], sets:[4,4,4,4,5,5,5,5,4,3,3,3], reps:["10","10","10","10","6","5","5","5","4","3","2-3","8"], backoff:[null,null,null,null,null,null,null,null,null,{sets:2,reps:"5",w:460},{sets:3,reps:"5",w:490},null], note:"Total load including the carriage. Subtract carriage weight, divide by 2 = plates per side." }
      ]},
      { label:"Superset 1", rest:"Rest 90s", supersetted:true, exercises:[
...
const BENCH_FIELDS = {
  chest_tri: [
    { id:"bench",    label:"Flat Barbell Bench Press",       hint:"4 × 10, 2 reps in reserve",    step:5,   min:45,  default:135, unit:"lbs" },
    { id:"incline",  label:"Incline DB Press (per hand)",    hint:"4 × 10",                        step:2.5, min:10,  default:45,  unit:"lbs" },
    { id:"pushdown", label:"Cable Tricep Pushdown",          hint:"4 × 12",                        step:2.5, min:10,  default:30,  unit:"lbs" },
    { id:"skull",    label:"EZ Bar Skull Crushers",          hint:"4 × 12",                        step:5,   min:20,  default:50,  unit:"lbs" },
    { id:"fly",      label:"Cable Fly (per side)",           hint:"3 × 12",                        step:2.5, min:5,   default:15,  unit:"lbs" },
    { id:"kickback", label:"Cable Kickback",                 hint:"3 × 12",                        step:2.5, min:5,   default:12.5,unit:"lbs" },
  ],
  back_shoulder_bi: [
    { id:"pullup_reps", label:"Pull-up max reps",           hint:"Bodyweight, strict form",       step:1,   min:0,   default:5,   unit:"reps" },
    { id:"row",      label:"Barbell Row",                   hint:"4 × 10",                        step:5,   min:45,  default:95,  unit:"lbs" },
    { id:"facepull", label:"Face Pull",                     hint:"4 × 12",                        step:2.5, min:10,  default:35,  unit:"lbs" },
    { id:"db_shoulder", label:"DB Shoulder Press (per hand)", hint:"3 × 10",                    step:2.5, min:10,  default:45,  unit:"lbs" },
    { id:"incline_curl", label:"Incline DB Curl (per hand)", hint:"4 × 10",                     step:2.5, min:10,  default:25,  unit:"lbs" },
    { id:"lat_raise", label:"Cable Lateral Raise (per side)", hint:"3 × 12",                    step:2.5, min:5,   default:10,  unit:"lbs" },
    { id:"lat_pulldown", label:"Lat Pulldown",              hint:"3 × 12",                        step:5,   min:30,  default:100, unit:"lbs" },
  ],
  legs: [
    { id:"trap_bar",  label:"Trap Bar Deadlift",            hint:"4 × 10",                        step:5,   min:45,  default:155, unit:"lbs" },
    { id:"leg_press", label:"Leg Press (total load w/ carriage)", hint:"4 × 10",               step:10,  min:100, default:250, unit:"lbs" },
    { id:"rdl",       label:"Romanian Deadlift",            hint:"3 × 12",                        step:5,   min:45,  default:95,  unit:"lbs" },
    { id:"calf",      label:"Standing Calf Raise",          hint:"3 × 15",                        step:5,   min:0,   default:70,  unit:"lbs" },
    { id:"leg_ext",   label:"Leg Extension (both legs)",    hint:"3 × 12",                        step:5,   min:20,  default:100, unit:"lbs" },
    { id:"leg_curl",  label:"Lying Leg Curl (both legs)",   hint:"3 × 12",                        step:5,   min:20,  default:80,  unit:"lbs" },
  ]
};

const defaultBenchmarks = () => {
  const b = {};
  Object.values(BENCH_FIELDS).flat().forEach(f => b[f.id] = f.default);
  return b;
};

// ── STEPPER ───────────────────────────────────────────
function Stepper({ label, hint, value, onChange, step, min = 0, unit = "lbs", accent }) {
  return (
    <div style={{background:"#101010",border:"1px solid #1C1C1C",borderRadius:"6px",padding:"12px",marginBottom:"8px"}}>
      <div style={{fontSize:"14px",fontWeight:700,color:"#D0D0D0",marginBottom:"2px",fontFamily:"'Barlow Condensed',sans-serif",letterSpacing:"0.3px"}}>{label}</div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
        ...
        <div style={{flex:1,textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:"22px",fontWeight:600,color:"#C0C0C0"}}>{value}<span style={{fontSize:"12px",color:"#555",marginLeft:"4px"}}>{unit}</span></div>
...
function OnboardingScreen({ session, onComplete }) {
  ...
  const [d1Dow, setD1Dow]           = useState(0);
  const [d2Dow, setD2Dow]           = useState(2);
  const [d3Dow, setD3Dow]           = useState(4);
  const [startDate, setStartDate]   = useState(new Date().toISOString().split('T')[0]);
  const [equipment, setEquipment]   = useState(defaultEquipment);
  const [benchmarks, setBenchmarks] = useState(defaultBenchmarks);
  const [saving, setSaving]         = useState(false);
  const accent = "#F97316";
  const STEPS = ["Schedule","Equipment","Chest & Tri","Back & Shoulders","Legs"];
  ...
  const handleFinish = async () => {
    ...
    await supabase.from("user_progress").upsert({
      user_id: session.user.id, benchmarks, equipment, setup_complete: true,
      start_date: startDate, day1_dow: d1Dow, day2_dow: d2Dow, day3_dow: d3Dow,
      current_week: 1, current_day: 'chest_tri', updated_at: new Date().toISOString()
    });
  ...
  const dayBtn = (val, set, i) => (
    <button key={i} onClick={() => set(i)} style={{flex:1,padding:"7px 2px",background:val===i?accent:"#1A1A1A",border:`1px solid ${val===i?accent:"#2A2A2A"}`,borderRadius:"3px",color:val===i?"#000":"#666",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"11px",fontWeight:val===i?700:400,cursor:"pointer",letterSpacing:"0.3px"}}>
      {DAYS[i]}
    </button>
  );
  ...
  return (
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:"90px"}}>
      ...
      <div style={{padding:"20px 16px 12px",borderBottom:"1px solid #1A1A1A"}}>
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#444",letterSpacing:"2px",marginBottom:"6px"}}>SETUP · STEP {step+1} OF {STEPS.length}</div>
        <div style={{fontSize:"26px",fontWeight:900,color:"#E0E0E0",letterSpacing:"0.5px",marginBottom:"10px"}}>{STEPS[step].toUpperCase()}</div>
        <div style={{display:"flex",gap:"4px"}}>
          {STEPS.map((_,i) => <div key={i} style={{flex:1,height:"2px",background:i<=step?accent:"#1A1A1A",borderRadius:"2px",transition:"background 0.3s"}} />)}
        </div>
      </div>
      ...
      <div style={{padding:"16px"}}>
        {/* Step 0: Schedule */}
        {step === 0 && <>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#555",marginBottom:"16px",lineHeight:1.6}}>Choose your workout days and start date. The app will auto-open to the right workout based on today's date.</div>
          {[{label:"Workout 1 — Chest & Tri",a:"#F97316",val:d1Dow,set:setD1Dow},{label:"Workout 2 — Back & Shoulders",a:"#3B82F6",val:d2Dow,set:setD2Dow},{label:"Workout 3 — Legs",a:"#22C55E",val:d3Dow,set:setD3Dow}].map(({label,a,val,set}) => (
            <div key={label} style={{marginBottom:"16px"}}>
              ...
              <div style={{display:"flex",gap:"3px"}}>{DAYS.map((_,i) => dayBtn(val,set,i))}</div>
            </div>
          ))}
        ...
        {/* Step 1: Equipment */}
        {step === 1 && <>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#555",marginBottom:"16px",lineHeight:1.6}}>Check everything available at your gym. The plan will provide exercise substitutions for any missing equipment.</div>
        ...
        {/* Steps 2-4: Benchmarks */}
        {step >= 2 && step <= 4 && <>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#555",marginBottom:"16px",lineHeight:1.6}}>Enter your working weight for the sets and reps shown — the weight you can do leaving 2 reps in reserve, not your max.</div>
          {Object.values(BENCH_FIELDS)[step-2].map(f => (
            <Stepper key={f.id} label={f.label} hint={f.hint} value={benchmarks[f.id]} onChange={v => setBench(f.id, v)} step={f.step} min={f.min} unit={f.unit} accent={accent} />
          ))}
        </>}
      </div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:"480px",padding:"12px 16px",background:"#080808",borderTop:"1px solid #1A1A1A",display:"flex",gap:"10px"}}>
        ...
        <button onClick={() => step < STEPS.length-1 ? setStep(s => s+1) : handleFinish()} disabled={saving} style={{flex:2,padding:"12px",background:saving?"#151515":accent,border:`1px solid ${saving?"#222":accent}`,borderRadius:"4px",color:saving?"#444":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,cursor:saving?"not-allowed":"pointer",letterSpacing:"1.5px"}}>
          {saving ? "GENERATING PLAN..." : step < STEPS.length-1 ? "NEXT →" : "GENERATE MY PLAN →"}
        </button>
      </div>
    </div>
  );
...
function SettingsScreen({ session, userProgress, onBack, onSave }) {
  ...
  const handleSave = async () => {
    ...
    const weights = buildWeights(benchmarks, equipment);
    await supabase.from("user_progress").upsert({
      user_id: session.user.id, benchmarks, equipment,
      start_date: startDate, day1_dow: d1Dow, day2_dow: d2Dow, day3_dow: d3Dow,
  ...
  const dayBtn = (val, set, i) => <button key={i} onClick={() => set(i)} style={{flex:1,padding:"6px 2px",background:val===i?accent:"#1A1A1A",border:`1px solid ${val===i?accent:"#2A2A2A"}`,borderRadius:"3px",color:val===i?"#000":"#666",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"11px",fontWeight:val===i?700:400,cursor:"pointer"}}>{DAYS[i]}</button>;
    ...
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:"90px"}}>
      ...
      <div style={{position:"sticky",top:0,zIndex:20,background:"#080808",borderBottom:"1px solid #1A1A1A",padding:"12px 16px 10px"}}>
        ...
        <div style={{display:"flex",gap:"4px",overflowX:"auto",paddingBottom:"2px"}}>
          {tabs.map(t => <button key={t.id} onClick={() => setSettingsTab(t.id)} style={{padding:"6px 10px",background:settingsTab===t.id?"#6B7280":"#111",border:`1px solid ${settingsTab===t.id?"#6B7280":"#222"}`,borderRadius:"3px",color:settingsTab===t.id?"#000":"#555",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"11px",fontWeight:settingsTab===t.id?700:400,cursor:"pointer",whiteSpace:"nowrap"}}>{t.label.toUpperCase()}</button>)}
        </div>
      </div>
      ...
      <div style={{padding:"16px"}}>
        ...
        {settingsTab === "equipment" && <>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#555",marginBottom:"12px",lineHeight:1.6}}>Uncheck any equipment not available at your gym. Save to regenerate the plan with substitutions.</div>
          {EQUIPMENT_LIST.map(eq => (
            <button key={eq.id} onClick={() => toggleEq(eq.id)} style={{width:"100%",display:"flex",alignItems:"center",gap:"12px",padding:"14px",marginBottom:"8px",background:equipment[eq.id]?accent+"12":"#101010",border:`1px solid ${equipment[eq.id]?accent:"#1C1C1C"}`,borderRadius:"6px",cursor:"pointer",textAlign:"left"}}>
              <div style={{width:"20px",height:"20px",borderRadius:"4px",border:`2px solid ${equipment[eq.id]?accent:"#333"}`,background:equipment[eq.id]?accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"12px",color:"#000",fontWeight:700}}>{equipment[eq.id]?"✓":""}</div>
              <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"16px",fontWeight:600,color:equipment[eq.id]?"#D0D0D0":"#555"}}>{eq.label}</span>
        ...
        {["chest","back","legs"].map((t, ti) => settingsTab === t && (
          <div key={t}>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#555",marginBottom:"12px",lineHeight:1.6}}>Update your benchmark weights. Save to regenerate all 12 weeks from these new baselines.</div>
...
export default function App() {
  ...
  const loadProgress = async () => {
    ...
    const { data: adjs } = await supabase.from("weight_adjustments").select("*").eq("user_id", session.user.id);
    if (adjs) {
      const map = {};
  ...
  const adjustW = (exId, wi, delta) => {
    ...
    setAdj(p => ({...p, [k]: next}));
    if (session) supabase.from("weight_adjustments").upsert({ user_id: session.user.id, day: tab, week, exercise_id: exId, weight: next, updated_at: new Date().toISOString() }, { onConflict:"user_id,day,week,exercise_id" });
  };
  const changeWeek = (w) => {
    const c = Math.max(1, Math.min(12, w));
    setWeek(c); setAiRes(null); setFeedback(""); setW1Feedback(""); setW1AiRes(null); setDone({}); skipTimer(); setMobOpen(true);
    if (session) supabase.from("user_progress").upsert({ user_id: session.user.id, current_week: c, updated_at: new Date().toISOString() });
  };
  const changeTab = (t) => {
    setTab(t); setAiRes(null); setFeedback(""); setW1Feedback(""); setW1AiRes(null); setDone({}); skipTimer(); setMobOpen(true);
  ...
  const applyW1Recal = async (adjustments) => {
    const n = {...adj};
    const rows = [];
    for (const [id, w] of Object.entries(adjustments)) {
      for (let wk = 2; wk <= 12; wk++) {
        const baseW = adj[`${tab}_w${wk}_${id}`] ?? (PLAN[tab].groups.flatMap(g => g.exercises).find(e => e.id === id)?.weights[wk-1] ?? w);
        const diff = w - getW(id, 0);
    ...
    setAdj(n); setW1AiRes(null); setW1Feedback("");
  ...
  const buildPrompt = (summary, fbText, ids, wi) =>
  ...
  const feedbackSection = (isFeedback, state, loadingState, handler, applyHandler, placeholder, btnLabel) => (
    <div style={{background:"#0D0D0D",border:"1px solid #1A1A1A",borderRadius:"6px",padding:"14px",marginBottom:"20px"}}>
      <div style={{fontSize:"13px",fontWeight:700,color:"#666",letterSpacing:"2px",marginBottom:"10px",fontFamily:"'Barlow Condensed',sans-serif"}}>{isFeedback ? "SESSION FEEDBACK" : "WEEK 1 DEBRIEF"}</div>
      {!isFeedback && <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#555",marginBottom:"10px",lineHeight:1.6}}>How did week 1 feel overall? Describe any exercises that were too heavy, too light, or that need form work. This recalibrates weeks 2-12.</div>}
      <textarea value={state} onChange={e => (isFeedback ? setFeedback : setW1Feedback)(e.target.value)} placeholder={placeholder} style={{width:"100%",minHeight:"80px",background:"#080808",border:"1px solid #222",borderRadius:"4px",color:"#D0D0D0",fontFamily:"'JetBrains Mono',monospace",fontSize:"12px",padding:"10px",resize:"vertical",lineHeight:1.5}} />
      <button onClick={handler} disabled={loadingState || !state.trim()} style={{marginTop:"8px",width:"100%",padding:"11px",background:loadingState||!state.trim()?"#151515":day.accent,border:`1px solid ${loadingState||!state.trim()?"#222":day.accent}`,borderRadius:"4px",color:loadingState||!state.trim()?"#444":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,letterSpacing:"2px",cursor:loadingState||!state.trim()?"not-allowed":"pointer"}}>
        {loadingState ? "ANALYZING..." : btnLabel}
      </button>
      {(isFeedback ? aiRes : w1AiRes) && (() => {
        const res = isFeedback ? aiRes : w1AiRes;
        const apply = isFeedback ? applyAi : applyW1Recal;
        return (
          <div style={{marginTop:"12px",padding:"12px",background:"#080808",border:`1px solid ${day.accent}33`,borderRadius:"4px"}}>
            <div style={{fontSize:"13px",color:"#AAA",marginBottom:"10px",lineHeight:1.5}}>{res.analysis}</div>
            {Object.keys(res.adjustments || {}).length > 0 ? (
              <>
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#444",marginBottom:"8px",letterSpacing:"1px"}}>{isFeedback ? "SUGGESTED CHANGES" : "PLAN RECALIBRATION — WEEKS 2-12"}</div>
                {Object.entries(res.adjustments).map(([id, nw]) => {
                  const foundEx = day.groups.flatMap(g => g.exercises).find(e => e.id === id);
                  const cw = getW(id, week-1); const diff = Math.round((nw - cw) * 10) / 10;
                  return (
                ...
                <button onClick={() => apply(res.adjustments)} style={{marginTop:"10px",width:"100%",padding:"9px",background:day.accent+"20",border:`1px solid ${day.accent}66`,borderRadius:"4px",color:day.accent,fontFamily:"'Barlow Condensed',sans-serif",fontSize:"13px",fontWeight:700,letterSpacing:"1.5px",cursor:"pointer"}}>
                  {isFeedback ? "APPLY ALL CHANGES" : "APPLY TO WEEKS 2-12"}
                </button>
              </>
    ...
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:timer?"180px":"100px"}}>
      <style>{`
        ...
        * { box-sizing:border-box; -webkit-tap-highlight-color:transparent; }
        textarea { caret-color:${day.accent}; } textarea:focus { outline:none; border-color:${day.accent} !important; }
      ...
      {/* COACHING NOTE */}
      <div style={{margin:"12px 16px 4px",padding:"10px 12px",borderLeft:`3px solid ${day.accent}`,background:day.accent+"0D",fontSize:"13px",color:"#888",lineHeight:1.4,fontFamily:"'Barlow Condensed',sans-serif"}}>{day.notes[wi]}</div>
      ...
      <div style={{padding:"0 16px",marginTop:"12px"}}>
              ...
              return (
                <div key={ex.id} style={{background:"#101010",border:"1px solid #1C1C1C",borderRadius:"6px",padding:"12px",marginBottom:"8px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
                    ...
                    {cues && <button onClick={() => setCueOpen(p => ({...p,[ex.id]:!p[ex.id]}))} style={{background:isOpen?day.accent+"22":"#1A1A1A",border:`1px solid ${isOpen?day.accent:"#2A2A2A"}`,borderRadius:"4px",color:isOpen?day.accent:"#555",width:"26px",height:"26px",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",flexShrink:0,marginLeft:"8px",lineHeight:1}}>ⓘ</button>}
                  </div>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:delta!==null?"8px":"10px"}}>
                    ...
                    <div style={{flex:1,display:"flex",alignItems:"center",gap:"6px",justifyContent:"flex-end"}}>
                      ...
                      <div style={{textAlign:"center",minWidth:"56px"}}>
                        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"24px",fontWeight:600,color:isAdj?day.accent:"#C0C0C0",lineHeight:1}}>{wLabel}<span style={{fontSize:"11px",color:"#555"}}>{wSuffix}</span></div>
                  ...
                  {delta !== null && <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:dColor,marginBottom:"10px",paddingBottom:"8px",borderBottom:"1px solid #1A1A1A"}}>{delta===0?`same as week ${week-1}`:delta>0?`+${delta}lbs vs week ${week-1}`:`${delta}lbs vs week ${week-1}`}</div>}
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:"6px"}}>
                    {Array(s).fill(null).map((_,si) => {
                      const k = `${tab}_w${week}_${ex.id}_s${si}`, isDone = done[k];
                      return <button key={si} onClick={() => { const wasOff = !isDone; setDone(p => ({...p,[k]:!p[k]})); if (wasOff && si === s-1) startTimer(group.rest, `${group.label} · ${ex.name}`); }} style={{width:"34px",height:"34px",borderRadius:"4px",background:isDone?day.accent:"#1A1A1A",border:`1px solid ${isDone?day.accent:"#2A2A2A"}`,color:isDone?"#000":"#555",fontFamily:"'JetBrains Mono',monospace",fontSize:"12px",fontWeight:isDone?700:400,cursor:"pointer",lineHeight:1}}>{isDone?"✓":si+1}</button>;
                    })}
                    {bo && Array(bo.sets).fill(null).map((_,si) => {
                      const k = `${tab}_w${week}_${ex.id}_bo${si}`, isDone = done[k];
                      return <button key={`bo${si}`} onClick={() => { const wasOff = !isDone; setDone(p => ({...p,[k]:!p[k]})); if (wasOff && si===bo.sets-1) startTimer(group.rest, `${group.label} · back-off`); }} style={{width:"34px",height:"34px",borderRadius:"4px",background:isDone?"#555":"#1A1A1A",border:`1px dashed ${isDone?"#555":"#252525"}`,color:isDone?"#000":"#444",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",cursor:"pointer",lineHeight:1}}>{isDone?"✓":`b${si+1}`}</button>;
                    })}
                  </div>
                </div>
              );
        ...
        {/* WEEK 1 DEBRIEF — appears only in week 1 */}
        {week === 1 && (
          <div style={{marginBottom:"8px",padding:"10px 12px",background:"#0C1A0C",border:"1px solid #22C55E33",borderRadius:"6px",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#22C55E55",letterSpacing:"1px"}}>
            WEEK 1 — Submit the debrief below after completing this workout to recalibrate weeks 2-12.
          </div>
        )}
        {week === 1 && feedbackSection(false, w1Feedback, w1Loading, handleW1Feedback, applyW1Recal, "How did this workout feel?\n\ne.g. \"bench was too heavy, row was too light, curls felt perfect\"", "RECALIBRATE WEEKS 2-12 →")}

        {/* SESSION FEEDBACK */}
        {feedbackSection(true, feedback, loading, handleFeedback, applyAi, "How did it feel?\n\ne.g. \"bench was too easy, got 12 reps each set\"", "ADJUST WEIGHTS →")}
      </div>
