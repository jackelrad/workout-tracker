...
const COMPLETE_MSGS = [
  "Consistency over intensity. You showed up today — that's what separates people who get results from people who don't.",
  "Every session is a vote for the person you're becoming. You just cast yours.",
  "The hard days are the most important ones. You just had one of them.",
  "Progress isn't linear, but effort is. You put in the work today.",
  "Most people skipped today. You didn't. That's the whole difference.",
  "You won't remember how you felt walking in. You'll remember how you felt walking out.",
  "Another session in the books. The compound interest on consistency is coming.",
  "Strength is built one session at a time. This was one of them.",
  "Showing up is the hardest part. You keep doing the hardest part.",
];
...
function saveToLocal(uid, doneState) {
  try {
    const filtered = {};
    Object.entries(doneState).forEach(([k,v])=>{ if(v) filtered[k]=true; });
    localStorage.setItem(getLocalKey(uid), JSON.stringify(filtered));
  } catch(e) {}
}
function loadFromLocal(uid) {
  try { const d=localStorage.getItem(getLocalKey(uid)); return d?JSON.parse(d):null; } catch(e) { return null; }
}
...
function BenchInput({ label, hint, value, onChange, step, min=0, unit="lbs", accent }) {
  ...
  const commit = () => {
    ...
    if(!isNaN(v)&&v>=min) onChange(Math.round(v/step)*step); else setRaw(String(value));
    setEditing(false);
  };
  return (
    <div style={{background:"#101010",border:"1px solid #1C1C1C",borderRadius:"6px",padding:"12px",marginBottom:"8px"}}>
      ...
      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#999",marginBottom:"10px"}}>{hint}</div>
      <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
        <button onClick={()=>onChange(Math.max(min,Math.round((value-step)/step)*step))} style={{background:"#1A1A1A",border:"1px solid #2A2A2A",color:"#888",width:"34px",height:"34px",borderRadius:"3px",cursor:"pointer",fontSize:"16px",lineHeight:1}}>−</button>
        {editing?(
          ...
          <div onClick={()=>{setRaw(String(value));setEditing(true);}} style={{flex:1,textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:"22px",fontWeight:600,color:"#D0D0D0",cursor:"pointer",padding:"6px"}}>
            {value}<span style={{fontSize:"12px",color:"#888",marginLeft:"4px"}}>{unit}</span>
          </div>
        )}
        <button onClick={()=>onChange(Math.round((value+step)/step)*step)} style={{background:"#1A1A1A",border:"1px solid #2A2A2A",color:"#888",width:"34px",height:"34px",borderRadius:"3px",cursor:"pointer",fontSize:"16px",lineHeight:1}}>+</button>
      </div>
    </div>
  );
}
...
function AuthScreen() {
  ...
  const inp={width:"100%",background:"#101010",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#D0D0D0",fontFamily:"'JetBrains Mono',monospace",fontSize:"13px",padding:"12px",marginBottom:"10px"};
  return (
    <div style={{background:"#080808",minHeight:"100vh",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Barlow Condensed','Impact',sans-serif",padding:"20px"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;600;700;900&family=JetBrains+Mono:wght@400;600&display=swap'); html,body{background:#080808;margin:0;padding:0;} *{box-sizing:border-box;} input:focus{outline:none;border-color:${accent}!important;}`}</style>
      <div style={{width:"100%",maxWidth:"360px"}}>
        <div style={{textAlign:"center",marginBottom:"8px"}}>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#888",letterSpacing:"3px",marginBottom:"8px"}}>12-WEEK STRENGTH PROGRAM</div>
          <div style={{fontSize:"34px",fontWeight:900,color:"#E0E0E0",letterSpacing:"1px",marginBottom:"8px"}}>WORKOUT TRACKER</div>
          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#AAA",lineHeight:1.6,marginBottom:"24px"}}>
        ...
        <div style={{display:"flex",marginBottom:"24px",border:"1px solid #2A2A2A",borderRadius:"6px",overflow:"hidden"}}>
          {["signin","signup"].map(m=>(
            <button key={m} onClick={()=>{setMode(m);setError("");}} style={{flex:1,padding:"10px",background:mode===m?accent:"transparent",border:"none",color:mode===m?"#000":"#888",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,letterSpacing:"1.5px",cursor:"pointer"}}>
        ...
        <button onClick={handle} disabled={loading} style={{width:"100%",padding:"13px",background:loading?"#151515":accent,border:`1px solid ${loading?"#222":accent}`,borderRadius:"4px",color:loading?"#666":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"15px",fontWeight:700,letterSpacing:"2px",cursor:loading?"not-allowed":"pointer"}}>
...
function OnboardingScreen({session,onComplete}) {
  ...
  const dayBtn=(val,set,i)=>(
    <button key={i} onClick={()=>set(i)} style={{flex:1,padding:"8px 2px",background:val===i?accent:"#1A1A1A",border:`1px solid ${val===i?accent:"#2A2A2A"}`,borderRadius:"3px",color:val===i?"#000":"#AAA",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"12px",fontWeight:val===i?700:400,cursor:"pointer"}}>
      {DAYS_SHORT[i]}
    </button>
  );
  ...
  return (
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:"90px"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;600;700;900&family=JetBrains+Mono:wght@400;600&display=swap'); html,body{background:#080808;margin:0;padding:0;} *{box-sizing:border-box;} input:focus{outline:none;} button:active{opacity:0.75;}`}</style>
      <div style={{padding:"20px 16px 12px",borderBottom:"1px solid #1A1A1A"}}>
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#888",letterSpacing:"2px",marginBottom:"6px"}}>SETUP · {step+1} OF {ONBOARDING_COPY.length}</div>
        <div style={{fontSize:"26px",fontWeight:900,color:"#E0E0E0",marginBottom:"4px"}}>{copy.title.toUpperCase()}</div>
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#AAA",lineHeight:1.6,marginBottom:"12px"}}>{copy.sub}</div>
        <div style={{display:"flex",gap:"4px"}}>{ONBOARDING_COPY.map((_,i)=><div key={i} style={{flex:1,height:"2px",background:i<=step?accent:"#1A1A1A",borderRadius:"2px",transition:"background 0.3s"}}/>)}</div>
      </div>
        ...
        {step===0&&(
          <>
            ...
            <div style={{marginTop:"8px"}}>
              <div style={{fontSize:"12px",fontWeight:700,color:"#BBB",letterSpacing:"1px",marginBottom:"6px"}}>PROGRAM START DATE</div>
        ...
        {step===1&&(
          <div>
            {EQUIPMENT_LIST.map(eq=>(
              <button key={eq.id} onClick={()=>setEquipment(p=>({...p,[eq.id]:!p[eq.id]}))} style={{width:"100%",display:"flex",alignItems:"center",gap:"12px",padding:"14px",marginBottom:"8px",background:equipment[eq.id]?accent+"12":"#101010",border:`1px solid ${equipment[eq.id]?accent:"#1C1C1C"}`,borderRadius:"6px",cursor:"pointer",textAlign:"left"}}>
                <div style={{width:"20px",height:"20px",borderRadius:"4px",border:`2px solid ${equipment[eq.id]?accent:"#444"}`,background:equipment[eq.id]?accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"12px",color:"#000",fontWeight:700}}>{equipment[eq.id]?"✓":""}</div>
                <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"16px",fontWeight:600,color:equipment[eq.id]?"#D0D0D0":"#888"}}>{eq.label}</span>
              </button>
            ))}
          </div>
        )}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:"480px",padding:"12px 16px",background:"#080808",borderTop:"1px solid #1A1A1A",display:"flex",gap:"10px"}}>
        {step>0&&<button onClick={()=>setStep(s=>s-1)} style={{flex:1,padding:"12px",background:"#1A1A1A",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#BBB",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,cursor:"pointer",letterSpacing:"1.5px"}}>← BACK</button>}
        <button onClick={()=>step<ONBOARDING_COPY.length-1?setStep(s=>s+1):handleFinish()} disabled={saving} style={{flex:2,padding:"12px",background:saving?"#151515":accent,border:`1px solid ${saving?"#222":accent}`,borderRadius:"4px",color:saving?"#666":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,cursor:saving?"not-allowed":"pointer",letterSpacing:"1.5px"}}>
...
function SettingsScreen({session,userProgress,onBack,onSave}) {
  ...
  const dayBtn=(val,set,i)=><button key={i} onClick={()=>set(i)} style={{flex:1,padding:"6px 2px",background:val===i?accent:"#1A1A1A",border:`1px solid ${val===i?accent:"#2A2A2A"}`,borderRadius:"3px",color:val===i?"#000":"#AAA",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"11px",fontWeight:val===i?700:400,cursor:"pointer"}}>{DAYS_SHORT[i]}</button>;
  return (
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:"90px"}}>
      <style>{`@import url('https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@400;600;700;900&family=JetBrains+Mono:wght@400;600&display=swap'); html,body{background:#080808;margin:0;padding:0;} *{box-sizing:border-box;} input:focus{outline:none;} button:active{opacity:0.75;}`}</style>
      <div style={{position:"sticky",top:0,zIndex:20,background:"#080808",borderBottom:"1px solid #1A1A1A",padding:"12px 16px 10px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px",marginBottom:"10px"}}>
          <button onClick={onBack} style={{background:"none",border:"1px solid #2A2A2A",color:"#AAA",width:"30px",height:"30px",borderRadius:"3px",cursor:"pointer",fontSize:"14px",lineHeight:1}}>←</button>
          <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#AAA",letterSpacing:"2px"}}>SETTINGS</span>
        </div>
        <div style={{display:"flex",gap:"4px",overflowX:"auto"}}>
          {tabs.map(t=><button key={t.id} onClick={()=>setStab(t.id)} style={{padding:"6px 10px",background:stab===t.id?"#6B7280":"#111",border:`1px solid ${stab===t.id?"#6B7280":"#222"}`,borderRadius:"3px",color:stab===t.id?"#000":"#AAA",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"11px",fontWeight:stab===t.id?700:400,cursor:"pointer",whiteSpace:"nowrap"}}>{t.label.toUpperCase()}</button>)}
        </div>
      </div>
      <div style={{padding:"16px"}}>
        {stab==="schedule"&&(
          <>
            ...
            <div>
              <div style={{fontSize:"12px",fontWeight:700,color:"#BBB",letterSpacing:"1px",marginBottom:"6px"}}>START DATE</div>
              <input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)} style={{width:"100%",background:"#101010",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#D0D0D0",fontFamily:"'JetBrains Mono',monospace",fontSize:"13px",padding:"10px 12px"}}/>
            </div>
          </>
        )}
        {stab==="equipment"&&(
          <div>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#AAA",marginBottom:"12px",lineHeight:1.6}}>Uncheck any equipment not at your gym. Save to regenerate the plan with substitutions.</div>
            {EQUIPMENT_LIST.map(eq=>(
              <button key={eq.id} onClick={()=>setEquipment(p=>({...p,[eq.id]:!p[eq.id]}))} style={{width:"100%",display:"flex",alignItems:"center",gap:"12px",padding:"14px",marginBottom:"8px",background:equipment[eq.id]?accent+"12":"#101010",border:`1px solid ${equipment[eq.id]?accent:"#1C1C1C"}`,borderRadius:"6px",cursor:"pointer",textAlign:"left"}}>
                <div style={{width:"20px",height:"20px",borderRadius:"4px",border:`2px solid ${equipment[eq.id]?accent:"#444"}`,background:equipment[eq.id]?accent:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:"12px",color:"#000",fontWeight:700}}>{equipment[eq.id]?"✓":""}</div>
                <span style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"16px",fontWeight:600,color:equipment[eq.id]?"#D0D0D0":"#888"}}>{eq.label}</span>
        ...
        {["chest","back","legs"].map((t,ti)=>stab===t&&(
          <div key={t}>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#AAA",marginBottom:"12px",lineHeight:1.6}}>Update your benchmarks. Save to regenerate all 12 weeks from these new baselines.</div>
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:"480px",padding:"12px 16px",background:"#080808",borderTop:"1px solid #1A1A1A"}}>
        <button onClick={handleSave} disabled={saving} style={{width:"100%",padding:"12px",background:saving?"#151515":accent,border:`1px solid ${saving?"#222":accent}`,borderRadius:"4px",color:saving?"#666":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,cursor:saving?"not-allowed":"pointer",letterSpacing:"2px"}}>
...
function HistoryScreen({session,adj,onBack}) {
  ...
  useEffect(()=>{
    async function load(){
      ...
      if(sets){
        const map={};
        sets.forEach(s=>{const key=`${s.day}_w${s.week}`;if(!map[key])map[key]={day:s.day,week:s.week,date:s.updated_at,count:0};map[key].count++;});
  ...
  return (
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:"40px"}}>
      <style>{`html,body{background:#080808;margin:0;padding:0;}`}</style>
      <div style={{position:"sticky",top:0,zIndex:20,background:"#080808",borderBottom:"1px solid #1A1A1A",padding:"12px 16px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"12px"}}>
          <button onClick={onBack} style={{background:"none",border:"1px solid #2A2A2A",color:"#AAA",width:"30px",height:"30px",borderRadius:"3px",cursor:"pointer",fontSize:"14px",lineHeight:1}}>←</button>
          <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#AAA",letterSpacing:"2px"}}>WORKOUT HISTORY</span>
        </div>
      </div>
      <div style={{padding:"16px"}}>
        {loading&&<div style={{textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#888",padding:"40px 0"}}>Loading history...</div>}
        {!loading&&sessions.length===0&&(
          <div style={{textAlign:"center",padding:"40px 0"}}>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#888",lineHeight:1.8}}>No completed sessions yet.<br/>Complete sets during a workout to see history here.</div>
          </div>
        )}
        {sessions.map((s,i)=>(
          <div key={i} style={{background:"#101010",border:"1px solid #1C1C1C",borderRadius:"6px",padding:"14px",marginBottom:"8px"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"6px"}}>
              <div>
                ...
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#AAA"}}>Week {s.week} · {PHASES[s.week-1]} · {s.count} sets logged</div>
              </div>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#888"}}>{formatDate(s.date)}</div>
            </div>
            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:dayAccent(s.day)+"BB"}}>{getKeyWeight(s.day,s.week)}</div>
          </div>
        ))}
      </div>
    </div>
    ...
    <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:"480px",zIndex:100,background:td?"#0D200D":"#0D0D0D",borderTop:`2px solid ${td?"#22C55E":accent}`,padding:"14px 20px",display:"flex",alignItems:"center",gap:"16px",boxShadow:"0 -4px 24px rgba(0,0,0,0.6)"}}>
      <div style={{position:"relative",width:"68px",height:"68px",flexShrink:0}}>
        <svg width="68" height="68" style={{transform:"rotate(-90deg)"}}>
          <circle cx="34" cy="34" r={r} fill="none" stroke="#2A2A2A" strokeWidth="4"/>
      ...
      <div style={{flex:1}}>
        ...
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#AAA",marginTop:"2px"}}>{label}</div>
        {!td&&<div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#888",marginTop:"4px"}}>{total}s total · {total-seconds}s elapsed</div>}
      </div>
      <button onClick={onSkip} style={{background:"#1A1A1A",border:"1px solid #333",color:"#AAA",borderRadius:"4px",padding:"8px 14px",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"13px",fontWeight:700,cursor:"pointer",flexShrink:0}}>{td?"CLOSE":"SKIP"}</button>
    </div>
  );
}
...
function CompletionOverlay({day,week,message,onClose}) {
  return (
    <div style={{position:"fixed",inset:0,background:"rgba(8,8,8,0.97)",zIndex:200,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"24px"}}>
      <div style={{width:"100%",maxWidth:"400px",textAlign:"center"}}>
        <div style={{fontSize:"72px",marginBottom:"12px",lineHeight:1}}>✓</div>
        <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"32px",fontWeight:900,color:"#E0E0E0",letterSpacing:"1px",marginBottom:"8px"}}>WORKOUT COMPLETE</div>
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:day.accent,letterSpacing:"1px",marginBottom:"20px"}}>WEEK {week} · {day.label.toUpperCase()} · {PHASES[week-1].toUpperCase()}</div>
        <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"12px",color:"#C0C0C0",lineHeight:1.8,marginBottom:"32px",padding:"0 8px"}}>{message}</div>
        <button onClick={onClose} style={{width:"100%",padding:"14px",background:day.accent,border:"none",borderRadius:"6px",color:"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"16px",fontWeight:700,letterSpacing:"2px",cursor:"pointer"}}>KEEP GOING →</button>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const loadProgress=async()=>{
    ...
    // Load completed sets: local first (fast), then Supabase (authoritative)
    const localDone=loadFromLocal(uid);
    if(localDone) setDone(localDone);
    const{data:sets}=await supabase.from("completed_sets").select("*").eq("user_id",uid).eq("completed",true);
    if(sets&&sets.length>0){
      const dMap={};
      sets.forEach(s=>{ dMap[`${s.day}_w${s.week}_${s.set_key}`]=true; });
      setDone(dMap);
      saveToLocal(uid,dMap);
    }
  };
  ...
  const saveWeight=(day,wk,exId,val)=>{
    if(!session) return;
    supabase.from("weight_adjustments").upsert({user_id:session.user.id,day,week:wk,exercise_id:exId,weight:val,updated_at:new Date().toISOString()},{onConflict:"user_id,day,week,exercise_id"}).then(({error})=>{ if(error) console.error("Weight save error:",error); });
  };

  const adjustW=(exId,wi,delta)=>{
    const next=Math.max(0,r25(getW(exId,wi)+delta));
    const k=`${tab}_w${week}_${exId}`;
    setAdj(p=>({...p,[k]:next}));
  ...
  const saveSetToDb=async(day,wk,setKey,completed)=>{
    if(!session) return;
    const{error}=await supabase.from("completed_sets").upsert({user_id:session.user.id,day,week:wk,set_key:setKey,completed,updated_at:new Date().toISOString()},{onConflict:"user_id,day,week,set_key"});
    if(error) console.error("Set save error:",error);
  };
  ...
  const completeNextSet=(exId,totalSets,groupRest,groupLabel,isBackoff=false)=>{
    ...
    setDone(p=>{const next={...p,[fullKey]:true}; saveToLocal(session?.user?.id,next); return next;});
  ...
  const undoLastSet=(exId,totalSets,isBackoff=false)=>{
    ...
    if(lastIdx===-1) return;
    setDone(p=>{const next={...p,[`${pre}${lastIdx}`]:false}; saveToLocal(session?.user?.id,next); return next;});
    saveSetToDb(tab,week,`${dbPre}${lastIdx}`,false);
  };

  const changeWeek=(w)=>{
    const c=Math.max(1,Math.min(12,w));
    setWeek(c); setAiRes(null); setFeedback(""); setW1Feedback(""); setW1AiRes(null); setMobOpen(true); skipTimer(); setEditingW(null);
  ...
  return (
    <div style={{background:"#080808",minHeight:"100vh",color:"#E0E0E0",fontFamily:"'Barlow Condensed','Impact',sans-serif",maxWidth:"480px",margin:"0 auto",paddingBottom:timer?"180px":"100px"}}>
      <style>{`
        ...
        html, body { background: #080808; margin: 0; padding: 0; min-height: 100vh; }
      ...
      <div style={{position:"sticky",top:0,zIndex:20,background:"#080808",borderBottom:"1px solid #1A1A1A",padding:"12px 16px 10px"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"10px"}}>
          <button onClick={()=>changeWeek(week-1)} style={{background:"none",border:"1px solid #2A2A2A",color:"#AAA",width:"30px",height:"30px",borderRadius:"3px",fontFamily:"inherit",fontSize:"16px",cursor:"pointer",lineHeight:1,flexShrink:0}}>‹</button>
          <div style={{flex:1,display:"flex",alignItems:"center",gap:"10px"}}>
            <div style={{background:day.accent+"15",border:`1px solid ${day.accent}33`,borderRadius:"6px",padding:"4px 10px",textAlign:"center",flexShrink:0}}>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"9px",color:day.accent+"99",letterSpacing:"1px",lineHeight:1}}>WEEK</div>
              <div style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:"28px",fontWeight:900,color:day.accent,lineHeight:1}}>{week}</div>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"9px",color:day.accent+"66",lineHeight:1}}>OF 12</div>
            </div>
            <div style={{flex:1}}>
              <div style={{display:"flex",justifyContent:"space-between",marginBottom:"4px"}}>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#BBB"}}>{day.label.toUpperCase()}</span>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:pct===100?day.accent:"#AAA"}}>{pct}% done</span>
              </div>
      ...
      {/* MOTIVATIONAL */}
      <div style={{margin:"12px 16px 4px",padding:"10px 14px",background:"#0D0D0D",borderRadius:"6px",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"15px",fontWeight:600,color:"#C0C0C0",letterSpacing:"0.3px",lineHeight:1.3}}>
        {motiv}
      </div>

      {/* COACHING NOTE */}
      <div style={{margin:"6px 16px 4px",padding:"10px 12px",borderLeft:`3px solid ${day.accent}`,background:day.accent+"0D",fontSize:"13px",color:"#C0C0C0",lineHeight:1.4,fontFamily:"'Barlow Condensed',sans-serif"}}>{day.notes[wi]}</div>
      ...
      <div style={{padding:"0 16px",marginTop:"12px"}}>
        ...
        <div style={{marginBottom:"20px"}}>
          <button onClick={()=>setMobOpen(p=>!p)} style={{width:"100%",background:"none",border:"none",padding:0,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
            <span style={{fontSize:"13px",color:"#C0C0C0",fontWeight:700,letterSpacing:"1.5px",fontFamily:"'Barlow Condensed',sans-serif"}}>PRE-WORKOUT MOBILITY</span>
            <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
              <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#AAA"}}>~2-3 min</span>
              <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"12px",color:"#AAA"}}>{mobOpen?"▲":"▼"}</span>
            </div>
          </button>
          {mobOpen&&mobility.map((move,mi)=>(
            <div key={mi} style={{background:"#0D0D0D",border:"1px solid #1A1A1A",borderRadius:"6px",padding:"12px",marginBottom:"6px"}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:"6px"}}>
                <span style={{fontSize:"15px",fontWeight:700,color:"#D0D0D0"}}>{move.name}</span>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#AAA"}}>{move.timing}</span>
              </div>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#BBB",lineHeight:1.6}}>{move.instructions}</div>
            </div>
          ))}
        </div>
        ...
        {day.groups.map((group,gi)=>(
          <div key={gi} style={{marginBottom:"20px"}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:"8px",alignItems:"center"}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
                <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#AAA"}}>{group.supersetted?"alternate":"all sets each"}</span>
                <button onClick={()=>startTimer(group.rest,`${group.label} · ${group.rest}`)} style={{background:"#151515",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#AAA",padding:"4px 10px",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",display:"flex",alignItems:"center",gap:"5px"}}>
            ...
            {group.exercises.map((ex)=>{
              const w=getW(ex.id,wi),prevW=getPrevW(ex.id,wi);
              ...
              const dColor=delta>0?"#22C55E":delta<0?"#EF4444":"#888";
                ...
                <div key={ex.id} style={{background:"#101010",border:"1px solid #1C1C1C",borderRadius:"6px",padding:"12px",marginBottom:"8px"}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"8px"}}>
                    <div style={{flex:1}}>
                      ...
                      {ex.note&&<div style={{fontSize:"10px",color:"#AAA",fontFamily:"'JetBrains Mono',monospace",marginTop:"4px",lineHeight:1.5}}>{ex.note}</div>}
                    </div>
                    {cues&&(
                      <button onClick={()=>setCueOpen(p=>({...p,[ex.id]:!p[ex.id]}))} style={{background:isOpen?day.accent+"22":"#1A1A1A",border:`1px solid ${isOpen?day.accent:"#2A2A2A"}`,borderRadius:"4px",color:isOpen?day.accent:"#AAA",width:"26px",height:"26px",cursor:"pointer",fontFamily:"inherit",fontSize:"13px",flexShrink:0,marginLeft:"8px",lineHeight:1}}>ⓘ</button>
                    )}
                  </div>

                  {isOpen&&cues&&(
                    <div className="cuebox" style={{marginBottom:"10px",padding:"10px 12px",background:"#0A0A0A",border:`1px solid ${day.accent}33`,borderRadius:"4px",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",lineHeight:1.6,color:"#C0C0C0"}}>
                      ...
                      <a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(ex.name+' exercise tutorial form')}`} target="_blank" rel="noreferrer" style={{display:"inline-block",background:"none",border:`1px solid ${day.accent}44`,borderRadius:"4px",color:day.accent+"CC",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",padding:"4px 10px",textDecoration:"none",letterSpacing:"0.5px"}}>
                  ...
                  <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:delta!==null?"8px":"10px"}}>
                    <div style={{textAlign:"center",minWidth:"40px"}}>
                      ...
                      <div style={{fontSize:"10px",color:"#AAA",marginTop:"2px"}}>SETS</div>
                    </div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",color:"#555",fontSize:"20px"}}>×</div>
                    <div style={{textAlign:"center",minWidth:"40px"}}>
                      <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"28px",fontWeight:600,color:"#D0D0D0",lineHeight:1}}>{r}</div>
                      <div style={{fontSize:"10px",color:"#AAA",marginTop:"2px"}}>REPS</div>
                    </div>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",color:"#555",fontSize:"14px"}}>@</div>
                    <div style={{flex:1,display:"flex",alignItems:"center",gap:"6px",justifyContent:"flex-end"}}>
                      <button onClick={()=>adjustW(ex.id,wi,-2.5)} style={{background:"#1A1A1A",border:"1px solid #2A2A2A",color:"#AAA",width:"26px",height:"26px",borderRadius:"3px",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",fontSize:"14px",lineHeight:1}}>−</button>
                      <div style={{textAlign:"center",minWidth:"60px"}}>
                        {editingW===ex.id?(
                          ...
                          <div onClick={()=>{setEditingW(ex.id);setEditVal(String(w));}} style={{cursor:"pointer"}}>
                            <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"24px",fontWeight:600,color:isAdj?day.accent:"#D0D0D0",lineHeight:1}}>
                              {wLabel}<span style={{fontSize:"11px",color:"#888"}}>{wSuffix}</span>
                            </div>
                            <div style={{fontSize:"9px",color:"#777",marginTop:"1px"}}>tap to edit</div>
                          </div>
                        )}
                      </div>
                      <button onClick={()=>adjustW(ex.id,wi,2.5)} style={{background:"#1A1A1A",border:"1px solid #2A2A2A",color:"#AAA",width:"26px",height:"26px",borderRadius:"3px",cursor:"pointer",fontFamily:"'JetBrains Mono',monospace",fontSize:"14px",lineHeight:1}}>+</button>
                  ...
                  {bo&&<div style={{marginBottom:"10px",padding:"6px 10px",background:"#0C0C0C",border:"1px dashed #252525",borderRadius:"4px",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#BBB"}}>Back-off: {bo.sets} × {bo.reps} @ {bo.w}lbs</div>}
                  ...
                  <div style={{background:"#0A0A0A",borderRadius:"4px",padding:"10px"}}>
                    <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"8px"}}>
                      ...
                      <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#BBB",marginLeft:"4px"}}>{completedCount}/{s} sets</span>
                    </div>
                    <div style={{display:"flex",gap:"6px"}}>
                      ...
                      {completedCount>0&&(
                        <button onClick={()=>undoLastSet(ex.id,s,false)} style={{padding:"9px 12px",background:"#1A1A1A",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#BBB",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",cursor:"pointer"}}>UNDO</button>
                      )}
                    </div>
                  </div>
                  ...
                  {bo&&(
                    <div style={{background:"#0A0A0A",borderRadius:"4px",padding:"10px",marginTop:"6px",border:"1px dashed #1A1A1A"}}>
                      <div style={{display:"flex",alignItems:"center",gap:"6px",marginBottom:"8px"}}>
                        ...
                        <span style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#BBB",marginLeft:"4px"}}>{boCount}/{bo.sets} back-off</span>
                      </div>
                      <div style={{display:"flex",gap:"6px"}}>
                        {boCount<bo.sets?(
                          <button onClick={()=>completeNextSet(ex.id,bo.sets,group.rest,`${group.label} back-off`,true)} style={{flex:1,padding:"8px",background:"#1A1A1A",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#BBB",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"12px",fontWeight:700,letterSpacing:"1px",cursor:"pointer"}}>
                            BACK-OFF SET {boCount+1}
                          </button>
                        ):(
                          <div style={{flex:1,padding:"8px",background:"#0D200D",border:"1px solid #22C55E22",borderRadius:"4px",textAlign:"center",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#22C55E66"}}>back-off done ✓</div>
                        )}
                        {boCount>0&&<button onClick={()=>undoLastSet(ex.id,bo.sets,true)} style={{padding:"8px 10px",background:"#1A1A1A",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#BBB",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",cursor:"pointer"}}>UNDO</button>}
                      </div>
                    </div>
                  )}
                </div>
        ...
        {/* CORE FINISHER */}
        <div style={{padding:"10px 14px",marginBottom:"16px",background:"#0D0D0D",border:"1px solid #1A1A1A",borderRadius:"6px",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#BBB"}}>
          + Core finisher — your standard routine
        </div>

        {/* FINISH WORKOUT BUTTON */}
        <button onClick={handleFinishWorkout} style={{width:"100%",padding:"14px",background:"#0A1A0A",border:`1px solid ${day.accent}55`,borderRadius:"6px",color:day.accent,fontFamily:"'Barlow Condensed',sans-serif",fontSize:"16px",fontWeight:700,letterSpacing:"2px",cursor:"pointer",marginBottom:"24px"}}>
          FINISH WORKOUT ✓
        </button>
        ...
        {week===1&&(
          <div style={{marginBottom:"20px"}}>
            <div style={{marginBottom:"8px",padding:"10px 12px",background:"#0C1A0C",border:"1px solid #22C55E22",borderRadius:"6px",fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#22C55E99",letterSpacing:"1px"}}>
            ...
            <div style={{background:"#0D0D0D",border:"1px solid #1A1A1A",borderRadius:"6px",padding:"14px"}}>
              <div style={{fontSize:"13px",fontWeight:700,color:"#C0C0C0",letterSpacing:"2px",marginBottom:"6px",fontFamily:"'Barlow Condensed',sans-serif"}}>WEEK 1 DEBRIEF</div>
              <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#BBB",marginBottom:"10px",lineHeight:1.6}}>How did this workout feel? Describe any exercises that were too heavy, too light, or that need form work. The AI will recalibrate weeks 2-12 based on your feedback.</div>
              {w1AiRes?.applied?(
                <div style={{padding:"12px",background:"#0A0A0A",border:"1px solid #22C55E33",borderRadius:"4px"}}>
                  ...
                  <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#C0C0C0",lineHeight:1.6}}>{w1AiRes.summary}. Weeks 2-12 have been updated.</div>
                </div>
              ):(
                <>
                  <textarea value={w1Feedback} onChange={e=>setW1Feedback(e.target.value)} placeholder={"How did it feel?\n\ne.g. \"bench was too heavy, row was too light, curls felt perfect\""} style={{width:"100%",minHeight:"80px",background:"#080808",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#D0D0D0",fontFamily:"'JetBrains Mono',monospace",fontSize:"12px",padding:"10px",resize:"vertical",lineHeight:1.5}}/>
                  <button onClick={handleW1Feedback} disabled={w1Loading||!w1Feedback.trim()} style={{marginTop:"8px",width:"100%",padding:"11px",background:w1Loading||!w1Feedback.trim()?"#151515":"#22C55E",border:`1px solid ${w1Loading||!w1Feedback.trim()?"#222":"#22C55E"}`,borderRadius:"4px",color:w1Loading||!w1Feedback.trim()?"#666":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,letterSpacing:"2px",cursor:w1Loading||!w1Feedback.trim()?"not-allowed":"pointer"}}>
                  ...
                  {w1AiRes&&!w1AiRes.applied&&(
                    <div style={{marginTop:"12px",padding:"12px",background:"#080808",border:"1px solid #22C55E33",borderRadius:"4px"}}>
                      <div style={{fontSize:"13px",color:"#C0C0C0",marginBottom:"10px",lineHeight:1.5}}>{w1AiRes.analysis}</div>
                      {Object.keys(w1AiRes.adjustments||{}).length>0?(
                        <>
                          <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#BBB",marginBottom:"8px",letterSpacing:"1px"}}>PLAN RECALIBRATION — WEEKS 1-12</div>
                          {Object.entries(w1AiRes.adjustments).map(([id,nw])=>{
                            ...
                            return (
                              <div key={id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #1A1A1A",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px"}}>
                                <span style={{color:"#C0C0C0"}}>{fe?.name??id}</span>
                                <span style={{color:"#22C55E"}}>{nw}lbs <span style={{color:diff>0?"#22C55E":diff<0?"#EF4444":"#888",marginLeft:"6px"}}>{diff>0?`+${diff}`:diff<0?`${diff}`:"="}</span></span>
                              </div>
                            );
                          })}
                      ...
                      ):<div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#BBB"}}>Weights on target — no changes needed.</div>}
                    </div>
                  )}
                </>
              )}
        ...
        {week>1&&(
          <div style={{background:"#0D0D0D",border:"1px solid #1A1A1A",borderRadius:"6px",padding:"14px",marginBottom:"20px"}}>
            <div style={{fontSize:"13px",fontWeight:700,color:"#C0C0C0",letterSpacing:"2px",marginBottom:"10px",fontFamily:"'Barlow Condensed',sans-serif"}}>SESSION FEEDBACK</div>
            <textarea value={feedback} onChange={e=>setFeedback(e.target.value)} placeholder={"How did it feel?\n\ne.g. \"bench was too easy, got 12 reps each set\""} style={{width:"100%",minHeight:"80px",background:"#080808",border:"1px solid #2A2A2A",borderRadius:"4px",color:"#D0D0D0",fontFamily:"'JetBrains Mono',monospace",fontSize:"12px",padding:"10px",resize:"vertical",lineHeight:1.5}}/>
            <button onClick={handleFeedback} disabled={loading||!feedback.trim()} style={{marginTop:"8px",width:"100%",padding:"11px",background:loading||!feedback.trim()?"#151515":day.accent,border:`1px solid ${loading||!feedback.trim()?"#222":day.accent}`,borderRadius:"4px",color:loading||!feedback.trim()?"#666":"#000",fontFamily:"'Barlow Condensed',sans-serif",fontSize:"14px",fontWeight:700,letterSpacing:"2px",cursor:loading||!feedback.trim()?"not-allowed":"pointer"}}>
            ...
            {aiRes&&(
              <div style={{marginTop:"12px",padding:"12px",background:"#080808",border:`1px solid ${day.accent}33`,borderRadius:"4px"}}>
                <div style={{fontSize:"13px",color:"#C0C0C0",marginBottom:"10px",lineHeight:1.5}}>{aiRes.analysis}</div>
                {Object.keys(aiRes.adjustments||{}).length>0?(
                  <>
                    <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:"10px",color:"#BBB",marginBottom:"8px",letterSpacing:"1px"}}>SUGGESTED CHANGES</div>
                    {Object.entries(aiRes.adjustments).map(([id,nw])=>{
                      ...
                      return (
                        <div key={id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"6px 0",borderBottom:"1px solid #1A1A1A",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px"}}>
                          <span style={{color:"#C0C0C0"}}>{fe?.name??id}</span>
                          <span style={{color:day.accent}}>{nw}lbs <span style={{color:diff>0?"#22C55E":diff<0?"#EF4444":"#888",marginLeft:"6px"}}>{diff>0?`+${diff}`:diff<0?`${diff}`:"="}</span></span>
                        </div>
                      );
                    })}
